		...
		if(isVisible && !mShader.hasMultipleDifferentValues && mShader.objectReferenceValue != null)
		{
			...
			if(CategoryFilter("MATCAP"))
			{
				if(isGeneratedShader)
				{
					...
					TCP2_Utils.ShaderKeywordToggle("TCP2_SKINNED_MESH", "MatCap Skinned Mesh Fix", "Fix scaling issue when using a Skinned Mesh with the MatCap feature", keywordsList, ref updateKeywords);
					EditorGUI.indentLevel--;
#endif
